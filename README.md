...
Engine-first milestone for `ps1-outrun-racing-sim`: an original PS1-style Three.js drift game with deterministic controls, an endless ridge road, traffic, city-overlook scenery, local cover art, boost/recovery play, route recovery gates, authored combo ladders, checkpoint timer pressure, generated arcade audio, drift scoring, telemetry hooks, and browser QA.
...
- Endless low-poly ridge highway that loops around the player, with switchbacks, elevation, width variation, road stripes, surface scuffs, section-authored roadside props, drift-zone gate landmarks, combo-ladder route markers, route recovery gates, deterministic traffic, and a city skyline below the overlook.
...
- HUD/debug panels for speed, distance, ridge section, time remaining, drift score, combo, best combo, authored combo-ladder progress, boost, authored drift-zone progress, recovery-gate state, rival gap, traffic range, collision count, near-misses, extension total, recovery state, top speed, current style, latest award, offroad time, lateral position, and run time.
- Drift-first score economy with clean driving, controlled slide scoring, authored drift-zone bonuses, authored combo-ladder clear bonuses, near-miss bonuses, combo growth, chain breaks, traffic/offroad risk, rival pressure, and bounded telemetry history.
...
- Unit tests for endless ridge generation, route-section pacing, car dynamics, telemetry, traffic contacts, combo-ladder clear/drop behavior, score economy behavior, and calibration traces.
- Playwright QA for canvas rendering, input response, HUD framing, seed-free startup, authored combo ladders, and browser-exposed drift/traffic telemetry.
...
- Gameplay pass: traffic density tuning and recovery-gate balance/playtest tuning.
