...
Engine-first milestone for `ps1-outrun-racing-sim`: an original PS1-style Three.js drift game with deterministic controls, an endless ridge road, traffic, city-overlook scenery, local cover art, boost/recovery play, checkpoint timer pressure, generated arcade audio, drift scoring, telemetry hooks, and browser QA.
...
- Endless low-poly ridge highway that loops around the player, with switchbacks, elevation, width variation, road stripes, surface scuffs, section-authored roadside props, drift-zone gate landmarks, deterministic traffic, and a city skyline below the overlook.
